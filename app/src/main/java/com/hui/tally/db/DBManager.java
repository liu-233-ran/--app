...
public class DBManager {
     ...
     * 读取数据库当中的数据，写入内存集合里
     *   kind :表示收入或者支出
     *   这是一个完整的数据库管理类 DBManager，负责处理记账App中所有与SQLite数据库的交互操作。
     * */
...
//        循环读取游标内容，存储到对象当中
    ...
    * */
     ...
     * 根据备注搜索收入或者支出的情况列表
     * */
    public static List<AccountBean> getAccountListByKeywordFromAccounttb(String keyword) {
        List<AccountBean> list = new ArrayList<>();
        // 使用参数化查询防止SQL注入
        String sql = "SELECT * FROM accounttb WHERE beizhu LIKE ? OR typename LIKE ?";
